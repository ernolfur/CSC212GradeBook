...
// Return pointer to node containing the highest value below 'data'
template <class T>
Node<T>* SkipList<T>::goTo(T data) {
...
template <typename T>
unsigned int SkipList<T>::numAfter(T data) {
    Node<T>* p = goTo(data);
    Node<T>* n = p->nexts.at(0);
